...
export const FocusStreak: React.FC = () => {
  ...
  // 🔍 DEBUG: Inject debug function into window for easy testing
  const handleDebugFocusStreak = async () => {
    if (!user?.uid) {
    ...
    console.log('📊 Current allWorkSessions count:', allWorkSessions.length);
    
    // Check what's in current sessions
    console.log('📦 Current sessions breakdown:', {
      total: allWorkSessions.length,
      withUTC: allWorkSessions.filter(s => s.startTimeUTC).length,
      legacyOnly: allWorkSessions.filter(s => !s.startTimeUTC).length,
      sample: allWorkSessions.slice(0, 5).map(s => ({
    ...
    try {
      const freshSessions = await getSessionsForYear(user.uid, selectedYear);
      setAllWorkSessions(freshSessions);
      
      console.log('📊 Fresh data result:', {
        totalSessions: freshSessions.length,
        breakdown: {
          withUTC: freshSessions.filter(s => s.startTimeUTC).length,
          legacyOnly: freshSessions.filter(s => !s.startTimeUTC).length
        },
      ...
      // Analysis
      const hasUTC = freshSessions.some(s => s.startTimeUTC);
      const hasLegacy = freshSessions.some(s => !s.startTimeUTC);
      
      console.log('🎯 Data source analysis:', {
        hasUTCData: hasUTC,
        hasLegacyData: hasLegacy,
        issue: !hasLegacy ? '❌ ONLY UTC DATA - LEGACY MISSING!' : 
               hasLegacy && hasUTC ? '✅ Both sources working' : 
               '⚠️ Only legacy data'
      });
      
      if (!hasLegacy) {
        console.log('🚨 PROBLEM IDENTIFIED: Focus Streak is not getting legacy data!');
        console.log('💡 This means the focusStreakStore.ts fix may not be active.');
      }
  ...
  React.useEffect(() => {
    if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
      window.debugFocusStreak = handleDebugFocusStreak;
      console.log('🔧 Focus Streak debug function injected! Run: debugFocusStreak()');
    }
  }, [handleDebugFocusStreak]);
